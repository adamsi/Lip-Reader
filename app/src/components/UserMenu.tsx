...
// Minimal settings menu (no accounts): the only action is changing the voice.
export default function UserMenu({ onChangeVoice }: { onChangeVoice?: () => void }) {
  ...
  if (!onChangeVoice) return null;

  return (
    <div className="relative" ref={ref}>
      {/* Same "Liquid Glass" recipe as the Run Agent / info buttons. */}
      <button
        onClick={() => setOpen((v) => !v)}
        className="relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-full border border-white/30 bg-black/55 text-sm font-semibold text-white backdrop-blur-2xl backdrop-saturate-[1.8] transition active:scale-[0.97] shadow-[inset_0_1px_1px_rgba(255,255,255,0.6),inset_0_-3px_8px_rgba(0,0,0,0.25),0_8px_28px_rgba(0,0,0,0.35)]"
        aria-label="Settings"
      >
        <span className="pointer-events-none absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b from-white/35 to-transparent" />
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="relative drop-shadow-sm">
